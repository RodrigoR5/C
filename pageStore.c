...
void buildHeader(char *sql, char *tableName, int qtdPages) {
    ...
    while(token != NULL) { // enquanto existir tokens
        ...
        if(strcmp(token, "char") == 0 || strcmp(token, "pk_char") == 0) { // compara se o token é um char
            if(strcmp(token, "pk_char") == 0)
                fieldType = 'X'; // tipo do atributo 
            else
            {
                fieldType = 'C'; // tipo do atributo 
            }
        ...
        } else if(strcmp(token, "int") == 0 || strcmp(token, "pk_int") == 0 ) { // compara se é int
            if(strcmp(token, "pk_int") == 0)
                fieldType = 'Y'; // tipo do atributo 
            else
            {
            ...
            i++;
        } else if(strcmp(token, "varchar") == 0 || strcmp(token, "pk_varchar") == 0 ) { // compara se é varchar
            if(strcmp(token, "pk_varchar") == 0)
                fieldType = 'Z'; // tipo do atributo
            else
            {   
                fieldType = 'V'; 
            }
...
void insertInto(char *sql, int numPage) { 
    ...
    token = strtok(attrSqlCopy, ","); // quebra o token de atributos usando o delimitador de virgula
  	...
  	for(int i = 0; i < qtdFields; i++) {     
      	...
      	if(attributes[i].type == 'C' || attributes[i].type == 'X') {
            insertSize += attributes[i].size;
        } else if(attributes[i].type == 'I' || attributes[i].type == 'Y' ) {
            insertSize += attributes[i].size;
        } else if(attributes[i].type == 'V' || attributes[i].type == 'Z') {
    ...
    for(int i = 0; i < head.qtdItems; i++) { // laço para percorrer os itens
        ...
        for(int j = 0; j < qtdFields; j++) {  
			// se o atributo for char
          	if(attributes[j].type == 'C' || attributes[j].type == 'X') { 
            ...
            } else if(attributes[j].type == 'I' || attributes[j].type == 'Y') {
                fread(&intInFile, sizeof(int), 1, page);
                printf("%d\t", intInFile);
            // se o atributo for varchar
            } else if(attributes[j].type == 'V' || attributes[j].type == 'Z') {
                charInFile = ' ';
